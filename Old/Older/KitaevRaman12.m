function out = KitaevRaman12(N,bins,flag,nn,jx,jz,h)
...
for ind = pts
   ...
   % ep = D(:,:,2,2);
    ...
    Bc = jx*hx + jy*hy*p3;
    deltac = 0*hx; %This term is assumed zero here
    jzc = jz*hz;
    ...
    for n=1:3
        ...
        R{n}(:,:,1,3) = 0;
        R{n}(:,:,1,4) = -Ac{n}-2i*deltac{n}+conj(Bc{n}); 
        R{n}(:,:,2,3) = conj(Ac{n})+2i*conj(deltac{n})-Bc{n};
        R{n}(:,:,2,4) = 0;   
        
        R{n}(:,:,3,1) = 0;
        R{n}(:,:,4,1) = conj( -Ac{n}-2i*deltac{n}+conj(Bc{n}) ); 
        R{n}(:,:,3,2) = conj( conj(Ac{n})+2i*conj(deltac{n})-Bc{n} );
        R{n}(:,:,4,2) = 0;  
