function out = hH0Raman_a3(N,bins,flag,nn,T,pbc,jx,jz,h)
...
%Build the matrix
DD = [jz,(jxa + jya.*p1).*conj(p3) ;
      (jxb + jyb.*p2),jz];
alph = kyb-kya.*conj(p3)-kxb*p2+kxa*p1/p3;
...
RDD = [jzc{n},(jxac{n} + jyac{n}.*p1).*conj(p3) ;
      (jxbc{n} + jybc{n}.*p2),jzc{n}];

%"p1=1" "p1*=0"
RD1 = [0,(jxac{n} + jyac{n}.*p1); 0,0];
%H0 = 1i*[FF , DD; -DD', -FF]; 
%H1 = 1i*[F1 , D1; zero, F2];
%%%%%%%%%%%%%%%%%%%%%% The BZ must be wrong. The Hamilonia are identical
