function out = hH0Raman_8_blah(N,bins,flag,nn,pbc,T,jx,jz,jx2,jz2,h)
...
%The six are the six symmetric binary products of R^aa, R^ac, R^cc
    %h1 = {aa, aa, aa, ac, ac, cc, ab, bc,[ac],[bc],[ac],[ac]};
    %h2 = {aa, ac, cc, ac, bc, cc, ab, bc,[ac],[bc],[bc],ac  };
m1 = {1,1,4,3,3,1,2,4,6,7,6,6 };
m2 = {1,3,5,3,4,4,4,4,6,7,7,3 };
mm = size(m1,2);
...
for zind = pts 
   ...
   % count1
    ...
    if (count >= count1+aleph) && flag && clockit2
        cl2 = clock;
        time = (cl2-cl1)*((4*N^2)/(count-count1))*nn;
...
jy2=jx2;
jxa = jx2; jxb = jx2; jya = jy2; jyb = jy2;
jxac = jxa*hxa; jxbc = jxa*hxb; jyac = jya*hya; 
jybc = jya*hyb; jzc = jz2*hz; 
for n =1:nm
    
HmD = [1i.*exp(-1i.*phi3).*jyac{n},0; 0,0];
HmF = 1i*[-kzac{n},kxac{n}*exp(-1i*phi3); 0, 0];
Hm = [HmF,HmD;0*HmD,-HmF.'];
Hp = Hm';

HD = [1i.*exp((1i*(-1)).*phi3).*jxac{n},1i.*jzc{n}; ...
        1i.*jzc{n},1i.*(jxbc{n}+exp(1i.*phi2).*jybc{n})];
    alph = -kybc{n} + kyac{n}*exp(-1i*phi3) + kxbc{n}*exp(-1i*phi2);% + kxa*exp(-1i*(phi3-phi1));
HF = 1i*[0,alph; -conj(alph), -kzbc{n}*(exp(1i*phi2)-exp(-1i*phi2))];
H0 = [HF,HD;conj(HD),-HF.'];
