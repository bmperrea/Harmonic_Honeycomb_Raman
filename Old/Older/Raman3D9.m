function out = Raman3D8(N,bins,flag,nn,Jx)
...
%         hx = 1/2;          hy = 1/2;           hz = 1; 
%     case 'xy'
%         hx = -1/sqrt(18);  hy = -1/sqrt(18);   hz = 0;    
%     case 'xz'
%         hx = -1/sqrt(6);   hy = 1/sqrt(6);     hz = 0;  
%     case 'yz'
%         hx = 1/sqrt(12);   hy = -1/sqrt(12);   hz = 0; 
%     case 'xx-xz'
%         hx = 1/3;          hy = 1/3;           hz = 0;
% end
...
hx1 = 1*Jx;   hy1 = 1*Jy;     hz1 = 0*Jz; 
hx2 = 1*Jx;   hy2 = 1*Jy;     hz2 = 0*Jz; 
factor2 = 2;
...
R(:,:,1,1) = hz/2; R(:,:,1,2) = 0; R(:,:,1,3) = (1/4)*(A-B); R(:,:,1,4) = (1/4)*(-A-B);
R(:,:,2,1) = 0; R(:,:,2,2) = -hz/2; R(:,:,2,3) = (1/4)*(A+B); R(:,:,2,4) = (1/4)*(-A+B);
R(:,:,3,1) = (1/4)*(conj(A)-conj(B)); R(:,:,3,2) = (1/4)*(conj(A)+conj(B)); R(:,:,3,3) = hz/2; R(:,:,3,4) = 0;
R(:,:,4,1) = (1/4)*(-conj(A)-conj(B)); R(:,:,4,2) = (1/4)*(-conj(A)+conj(B)); R(:,:,4,3) = 0; R(:,:,4,4) = -hz/2;
...
R(:,:,4,1) = (1/4).*((-1).*conj(A)+(-1).*conj(B)); R(:,:,4,2) = (1/4).*((-1).*conj(A)+conj(B)); R(:,:,4,3) = 0; R(:,:,4,4) = (-hz/2);
    ...
    Wpp = pi*real( VV(:,:,1,2).*VV2(:,:,2,1) + VV(:,:,2,1).*VV2(:,:,1,2) )/factor2;
    Wmm = pi*real( VV(:,:,3,4).*VV2(:,:,4,3) + VV(:,:,4,3).*VV2(:,:,3,4) )/factor2;
    Wpm = pi*real(( VV(:,:,1,4).*VV2(:,:,4,1) + VV(:,:,4,1).*VV2(:,:,1,4) ) ...
        + ( VV(:,:,3,2).* VV2(:,:,2,3) + VV(:,:,2,3).* VV2(:,:,3,2) ) )/factor2;
  %  [max(Wpp(:)),max(Wmm(:)),max(Wpm(:))]
