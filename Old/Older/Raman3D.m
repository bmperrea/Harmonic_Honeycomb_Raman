function [Evp,DEp,Ipp,Evm,DEm,Imm,Ev,DE,Ipm]= Raman3D(bins,N)
...
for y = ((1-N):(N-1))*29*pi/(36*sqrt(2)*N)
 ...
 %   ep = ep + em;
    ...
    Wpp = 2*pi*reshape( abs(V(:,:,1,2)).^2 ,(L)^2,1);
    Wmm = 2*pi*reshape( abs(V(:,:,3,4)).^2 ,(L)^2,1);
    Wpm = 2*pi*reshape( abs(V(:,:,1,4)).^2 + abs(V(:,:,2,3)).^2 ,(L)^2,1);
