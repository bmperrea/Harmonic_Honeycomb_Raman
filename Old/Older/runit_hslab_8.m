...
%Kitaev Hyperhoneycomb (H0) model
function I = runit_hslab_8(T,pbc,N,Jxx,h)
...
for n = 1:nn
    ...
    out = hH0Raman_8(N,bins,flag,nn,pbc,T,Jx,Jz,h);
...
%set(gca,'YTick',2*(0:5));
%hold off;
filename = ['H0_1DOS_Slab_',num2str(T),'_p_',num2str(pbc),'_Jx_',num2str(round(100*Jx))];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);

%H0_Raman_Slab_64_p_',num2str(pbc),

%Plot the ramans
set(0,'defaultAxesFontSize',fsz);
hh=figure;%('Position',position);
hold on;
plot(Ev,I{3},Ev,I{4},Ev,-I{5},Ev,I{6},Ev,I{7},Ev,I{8}/3,Ev,I{9},Ev,I{10}/2);
%errorbar(Ev,Ipp+Imm+Ipm,errs(:,4)+errs(:,5)+errs(:,6));
...
hold off;
filename = ['H0_Raman_Slab_',num2str(T),'_p_',num2str(pbc),'_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100)];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
hold off;
filename = ['H0_Raman_',num2str(T),'_p_',num2str(pbc),'_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_3'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
hold off;
filename = ['H0_Raman_',num2str(T),'_p_',num2str(pbc),'_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_2'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
xlabel('\omega/J_z');
ylabel('I(\omega)');
legend({'I_{[a,b]}'}, 'Location', 'NorthEast');
hold off;
filename = ['H0_Raman_',num2str(T),'_p_',num2str(pbc),'_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_a'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
%Print the ratios
disp(mean(I{5}(I{3}>0)./I{3}(I{3}>0)))
disp(mean(I{8}(I{3}>0)./I{3}(I{3}>0)))
disp(mean(I{7}(I{4}>0)./I{4}(I{4}>0)))
disp(mean(I{10}(I{6}>0)./I{6}(I{6}>0)))
disp(mean(I{12}(I{11}>0)./I{11}(I{11}>0)))
