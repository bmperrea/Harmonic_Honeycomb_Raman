function out = hH0Raman_6(N,bins,flag,nn,pbc,T,jx,jz,h)
...
HmD = [0,1i.*exp(-1i.*phi3).*jya; 0,0];
    %alp = kxa*exp(-1i*phi3);
HmF = 1i*[-kza,kxa*exp(-1i*phi3); 0, 0];
HmF2 = 1i*[0,kxa*exp(-1i*phi3); 0, -kza];
Hm = [HmF,HmD;HmD',-HmF2];
Hp = Hm';

HD = [1i.*jz,1i.*exp((1i*(-1)).*phi3).*jxa; ...
        1i.*(jxb+exp(1i.*phi2).*jyb),1i.*jz];
