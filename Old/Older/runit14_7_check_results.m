...
for n = 1:nn
    ...
    out = hH0Raman_tdl(N,bins,flag,nn,Jx,Jz,h);
    Ev = out{1}; %Same every time
    Dt{n}  = out{2}; %D = [Dpp,Dmm,Dpm]
    for m=1:8
        Ivt{m,n} = out{m+3}; % I{m} = [Ipp{m},Imm{m},Ipm{m}];
        It{m,n} =Ivt{m,n};% = Ivt{m,n}(1) + Ivt{m,n}(2) + Ivt{m,n}(3);
    end
end
...
%set(gca,'YTick',2*(0:5));
%hold off;
filename = ['3D_DOS_2_Jx_',num2str(round(100*Jx)),'_h_',num2str(100*h),'_99'];
saveas(hh,filename)
...
% %set(gca,'YTick',2*(0:5));
% hold off;
% filename = ['3D_DOS_2_Jx_',num2str(round(100*Jx)),'_h_',num2str(100*h)];
% saveas(hh,filename)
% print(hh, '-dpng', filename);
% print(hh, '-depsc', filename);
% 
...
hold off;
filename = ['3D_Raman_2_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_99'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
hold off;
filename = ['3D_Raman_2_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_993'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
hold off;
filename = ['3D_Raman_2_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_992'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
