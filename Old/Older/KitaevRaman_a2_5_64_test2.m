...
M = 4*T;
S = 2*T;
P = round(T/4);%Want smaller than T and divisible by 2
Q = 2*P;
...
%             B-conj(A)-2i*conj(d),1i*(a+b),-B-conj(A),1i*(-a+b+2i*jz)];
   ...
   z2 = [zero, zero, zero, zero, zero, zero, zero, zero;...
    ...
    % tic
     ...
     %Here I solve for twice as many eigenvalues as I will use just because
     %otherwise I tend to not have opposite pairs
    [V,D] = eig(H);
...
%     V = V(:, II);
    ...
    for n =1:3
        ...
        R2{n} = [-1i*b2c{n},conj(B2c{n}),1i*(bc{n}),2i*d2c{n}+conj(B2c{n});...
            B2c{n},1i*bc{n}+2*jzc{n},-B2c{n}-2i*conj(d2c{n}),1i*(b2c{n});...
 ...
 %tic
   ...
   % tic
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % This multiplication call takes up 3 times more time than the
            % second slowest thing - matrix diagonalization
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%tic
    ...
    for n=1:3
        RRr{n} = RR{n}(P+1:Q,1:P);
        RRr1{n} = RR1{n}(P+1:Q,1:P);
 %       RRr2{n} = RR2{n}(S+1:M,1:S);
    end
    
    
    ent = repmat(en2,1,P) + repmat(en2',P,1);
