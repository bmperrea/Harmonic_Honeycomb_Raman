...
for n = 1:nn
    ...
    out = KitaevRaman13_inf_p(N,bins,flag,nn,Jx,Jz,h);
...
title(['DOS for HyperHoneycomb Kitaev spinons inf_p_: J_x=',num2str(Jx),', h=',num2str(h)])
xlabel('\omega/J_z');
ylabel('density of states');
%set(gca,'XTick',-3:3); 
%set(gca,'YTick',2*(0:5));
%hold off;
filename = ['3D_DOS_inf_p_10^7_Jx_',num2str(round(100*Jx)),'_h_',num2str(100*h),'_2'];
saveas(hh,filename)
...
%set(gca,'YTick',2*(0:5));
hold off;
filename = ['3D_DOS_inf_p_10^7_Jx_',num2str(round(100*Jx)),'_h_',num2str(100*h)];
saveas(hh,filename)
print(hh, '-dpng', filename);
...
if h==0 && Jxx ==1
    ...
    hold off;
    filename = ['3D_DOS_inf_p_10^7_Jx_',num2str(round(100*Jx)),'_h_',num2str(100*h),'_5'];
    saveas(hh,filename)
    print(hh, '-dpng', filename);
end
