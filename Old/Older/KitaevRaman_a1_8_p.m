function out = KitaevRaman_a1_8(N,bins,flag,nn,jx,jz,h)
...
for xind = pts
    ...
    for yind = pts
        x = (xind + r1)*pi*sqrt(1/3)/N;
        y = (yind + r2)*pi*sqrt(3/29)/N; 
...
%             B-conj(A)-2i*conj(d),1i*(a+b),-B-conj(A),1i*(-a+b+2i*jz)];
   ...
   H2 =    [-1i*kz,jy*conj(p3),-1i*kz,(2i*kx-jy)*conj(p3);...
    ...
    zero = zeros(4);
     ...
     H = [H1, H2, zero, zero, zero, zero, zero,  H2';...
         ...
         zero, zero, zero, zero, zero, H2', H1, H2; ...
          H2, zero, zero, zero, zero, zero, H2', H1]/2;
...
R{n} = [R1{n}, R2{n}, zero, zero, zero, zero, zero, R2{n}';...
       ...
       R2{n}, zero, zero, zero, zero, zero, R2{n}', R1{n}]/2;

        %For the slab we c{n}an also define an edge Raman operator
 Re{n} = [R1{n}, zero, zero, zero, zero, zero, zero, zero;...
...
ylabel('k_y');
hold off;
filename = ['Fermi_surface_8_Jx_',num2str(round(100*jx)),'_h_',num2str(h*100)];
saveas(hh,filename)
