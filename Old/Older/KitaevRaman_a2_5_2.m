function out = KitaevRaman_a2_5_2(N,bins,flag,nn,jx,jz,h)
...
%          zero, zero, H2', H1]/2;
    ...
    for n =1:3
        ...
        Re1{n} = R1{n}/2; %[R1{n}, zeroc{n}, zeroc{n}, zero; ...
...
%              zeroc{n}, zeroc{n}, zeroc{n}, zeroc{n}; ...
%              zeroc{n}, zeroc{n}, zeroc{n}, zeroc{n}; ...
%              zeroc{n}, zeroc{n}, zeroc{n}, R1{n}]/2;
...
%     end
 ...
 %       RRr2{n} = RR2{n}(S+1:M,1:S);
    ...
    for m =1:6;
        W{m}(xind+N+1,yind+N+1,:,:) = pi*real(conj(RRr{m1{m}}).*RRr{m2{m}} ...
            + conj(RRr{m2{m}}).*RRr{m1{m}});
        W1{m}(xind+N+1,yind+N+1,:,:) = pi*real(conj(RRr1{m1{m}}).*RRr1{m2{m}} ...
 ...
 %           + RRr2{m2{m}}.*RRr2{m1{m}});
    ...
    enty(xind+N+1,yind+N+1,:,:) = ent;              
     ...
     if h>0
        ecut = 0.4*h*(100/N)^(3/2)*(8/sqrt(T));
    else
        ecut = (.015)*(100/N)*(8/sqrt(T));
    end
...
ylabel('k_y');
hold off;
filename = ['Fermi_surface_2_Jx_',num2str(round(100*jx)),'_h_',num2str(h*100)];
saveas(hh,filename)
