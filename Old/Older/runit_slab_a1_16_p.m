...
for n = 1:nn
    ...
    out = KitaevRaman_a1_16_p(N,bins,flag,nn,Jx,Jz,h);
...
title(['DOS for Slab2: J_x=',num2str(Jx),', h=',num2str(h)])
xlabel('\omega/J_z');
ylabel('density of states');
%set(gca,'XTick',-3:3); 
%set(gca,'YTick',2*(0:5));
hold off;
filename = ['3D_DOS_Slab2_16_p_Jx_',num2str(round(100*Jx)),'_h_',num2str(100*h),'_2'];
saveas(hh,filename)
...
%errorbar(Ev,Ipp+Imm+Ipm,errs(:,4)+errs(:,5)+errs(:,6));
title(['Raman Spectrum for Slab2: J_x=',num2str(Jx),', h=',num2str(h)])
xlabel('\omega/J_z');
ylabel('I(\omega)');
legend({'I_{aa}', 'I_{aa,ac}', 'I_{aa,cc}','I_{ac}','I_{ac,cc}','I_{cc}'}, 'Location', 'NorthEast');
hold off;
filename = ['3D_Raman_Slab2_16_p_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100)];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
hold off;
filename = ['3D_Raman_Slab2_16_p_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_3'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
hold off;
filename = ['3D_Raman_Slab2_16_p_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_2'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
%errorbar(Ev,Ipp+Imm+Ipm,errs(:,4)+errs(:,5)+errs(:,6));
title(['Edge1 Raman Spectrum for Slab2: J_x=',num2str(Jx),', h=',num2str(h)])
xlabel('\omega/J_z');
ylabel('I(\omega)');
legend({'I_{aa}', 'I_{aa,ac}', 'I_{aa,cc}','I_{ac}','I_{ac,cc}','I_{cc}'}, 'Location', 'NorthEast');
hold off;
...
% %errorbar(Ev,Ipp+Imm+Ipm,errs(:,4)+errs(:,5)+errs(:,6));
% title(['Edge2 Raman Spectrum for Slab2: J_x=',num2str(Jx),', h=',num2str(h)])
% xlabel('\omega/J_z');
% ylabel('I(\omega)');
% legend({'I_{aa}', 'I_{aa,ac}', 'I_{aa,cc}','I_{ac}','I_{ac,cc}','I_{cc}'}, 'Location', 'NorthEast');
% hold off;
