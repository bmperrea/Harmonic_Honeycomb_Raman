function out = KitaevRaman14(N,bins,flag,nn,jx,jz,h)
...
jy=jx;
%jy = 1.5*jx;  %Jy is always same as Jx
emax = 4*(jx+2*jy+jz) + 8*h;
...
%    
    ...
    Ac = 1.5*jx*hxa*p2 + jy*hya*p1;
%    Ac = 1.5*Ac;
    Bc = jx*hxb + 1.5*jy*hyb*conj(p3);
