function out = KitaevRaman_a2_5_32_test(N,bins,flag,nn,jx,jz,h)
...
%             B-conj(A)-2i*conj(d),1i*(a+b),-B-conj(A),1i*(-a+b+2i*jz)];
   ...
   H2 =    [-1i*b2,conj(B2),1i*(b),2i*d2+conj(B2);...
   ...
   Hd = [R1{n}, R2{n}, zero, zero;...
       R2{n}', R1{n}, R2{n}, zero; ...
       zero, R2{n}', R1{n}, R2{n}; ...
       zero, zero, R2{n}', R1{n}]/2;
   
   Hd2 = [zero, zero, zero, zero; ...
          zero, zero, zero, zero; ...
          zero, zero, zero, zero; ...
         R2{n}, zero, zero, zero]/2;
...
ylabel('k_y');
hold off;
filename = ['Fermi_surface_32_Jx_',num2str(round(100*jx)),'_h_',num2str(h*100)];
saveas(hh,filename)
