function out = hH0Raman_ptdl(N,bins,flag,nn,T,jx,jz,h)
...
for zind = pts 
   ...
   % count1
    ...
    if (count >= count1+aleph) && flag && clockit2
        cl2 = clock;
        time = (cl2-cl1)*((4*T*N^2)/(count-count1))*nn*.62;
...
enty(layind+N+1,xind+N+1,:,:) = ent;
enty2(layind+N+1,xind+N+1,:) = en2;
...
for n =1:nm
RD = [jzc{n},(jxac{n} + jyac{n}.*p1)./p3 ;
      (jxbc{n} + jybc{n}.*p2),jzc{n}];
R = 1i*[zero , RD; -RD', zero]; 
...
for m =1:mm;
    W{m}(layind+N+1,xind+N+1,:,:) = pi*real(conj(RRr{m1{m}}).*RRr{m2{m}} ...
        + conj(RRr{m2{m}}).*RRr{m1{m}});
end

else %If sn<0 then the energy is put in as -1.
     enty(layind+N+1,xind+N+1,:,:) = -ones(S);
     enty2(layind+N+1,xind+N+1,:) = -ones(S,1);
end
    end
    end
