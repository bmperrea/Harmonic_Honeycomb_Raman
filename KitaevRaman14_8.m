function out = KitaevRaman14_8(N,bins,flag,nn,jx,jz,h)
...
for ind = pts
    ...
    A = (jxa + jya.*p1)./p3;
    B = (jxb + jyb.*p2);
    d = ky*(1-p2) + kx*(p2-p3);%%%%%%%%%%%%%% Need to be revised in new coords.
...
%    
    ...
    Ac = (jxa*hxa + jya*hya*p1)./p3;
    Bc = (jxb*hxb + jyb*hyb.*p2);
    dc = 0*hz; %This term is assumed zero here
    ac = 0*hz; bc = 0*hz;
    jzc = jz*hz;
...
%     end
    ...
    for m=1:mm
        ...
        Wmm = pi*real(RR{m1{m}}(:,:,1,3).*RR{m2{m}}(:,:,3,1) ...
                + RR{m2{m}}(:,:,1,3).*RR{m1{m}}(:,:,3,1));  %These are right now, but not in earlier versions (mm <-> pp)
        Wpp = pi*real(RR{m1{m}}(:,:,2,4).*RR{m2{m}}(:,:,4,2) ... 
