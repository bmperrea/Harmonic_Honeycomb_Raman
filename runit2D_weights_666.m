...
did = 0:40/4; s12=did;s22=did;Jxx =did; Jzz=did;th2=did;
for n = (1:41);
    th = (n-1)*pi/160; th2(n)=th*2/pi;
    Jx = 3*tan(th)/(1+2.5*tan(th));
    Jz = 3/(1+2.5*tan(th));
    Jxx(n) = Jx;
    Jzz(n) = Jz;    
    ...
    if Jx ~= 1
        ...
        xlabel('\omega/J^z');
        ylabel('I');
        filename = ['2D_Raman3_Jx_',num2str(round(100*Jx)),'Jy_125_','_h_',num2str(100*h)];
        saveas(hh,filename)
        ...
        plot(Ev,Ixx);
        %errorbar(Ev,Ipp+Imm+Ipm,errs(:,4)+errs(:,5)+errs(:,6));
        title(['2D Raman: J_x=',num2str(Jx),'Jy_125_',', h=',num2str(h)])
        xlabel('\omega/J^z');
        ylabel('I');
...
hold off;
filename = ['2D_Raman_weights_Jy_125'];
saveas(hh,filename)
print(hh, '-dpng', filename);
