...
function I = runit_H1_pi_r(Jxx,h,~)
...
for n = 1:nn
    ...
    out = KitaevRaman_H1_pi_r2_checking(N,bins,flag,nn,Jx,Jz,h);
...
%title(['DOS for H-1\pi spinons: J_x=',num2str(Jx),', h=',num2str(h)])
xlabel('\omega/J_z');
ylabel('DOS');
%set(gca,'XTick',-3:3); 
%set(gca,'YTick',2*(0:5));
%hold off;
filename = ['H1_pi_DOS_',num2str(N),'_Jx_',num2str(round(100*Jx)),'_28'];
saveas(hh,filename)
...
%errorbar(Ev,Ipp+Imm+Ipm,errs(:,4)+errs(:,5)+errs(:,6));
%title(['Raman Spectrum for H-1\pi spinons: J_x=',num2str(Jx),', h=',num2str(h)])
xlabel('\omega/J_z');
ylabel('I(\omega)');
legend({'I_{aa}', 'I_{aa,ac}', '-I_{aa,cc}','I_{ac}','I_{ac,cc}','I_{cc}/3','I_{ab}','I_{bc}/2'}, 'Location', 'NorthEast');
hold off;
filename = ['H1_pi_Raman_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_028'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
hold off;
filename = ['H1_pi_Raman_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_328'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
...
hold off;
filename = ['H1_pi_Raman_Jx_',num2str(round(100*Jx)),'_h_',num2str(h*100),'_228'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
