...
R(:,:,4,1) = (1/4).*((-1).*conj(A)+(-1).*conj(B)); R(:,:,4,2) = (1/4).*((-1).*conj(A)+conj(B)); R(:,:,4,3) = 0; R(:,:,4,4) = (-hz/2);
    ...
    Wpp = pi*abs( VV(:,:,1,2).*VV2(:,:,2,1) + VV(:,:,2,1).*VV2(:,:,1,2) );
    Wmm = pi*abs( VV(:,:,3,4).*VV2(:,:,4,3) + VV(:,:,4,3).*VV2(:,:,3,4) );
    Wpm = pi*(abs( VV(:,:,1,4).*VV2(:,:,4,1) + VV(:,:,4,1).*VV2(:,:,1,4) ) ...
        + abs( VV(:,:,3,2).* VV2(:,:,2,3) + VV(:,:,2,3).* VV2(:,:,3,2) ) );
          
    [histw, histv] = histwv(2*ep(sn>=0),Wpp(sn>=0),0,2*emax,bins);
    Ipp = Ipp + histw;
    DEp = DEp + histv;
    
    [histw, histv] = histwv(2*em(sn>=0),Wmm(sn>=0),0,2*emax,bins);
    Imm = Imm + histw;
    DEm = DEm + histv;
    
    [histw, histv] = histwv(ep(sn>=0)+em(sn>=0),Wpm(sn>=0),0,2*emax,bins);
    Ipm = Ipm + histw;
    DE  = DE  + histv;
