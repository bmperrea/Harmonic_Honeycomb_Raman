...
legend({'I_{aa}/I_{ac}','I_{ab}/I_{ac}'}, 'Location', 'NorthEast');
hold off;
filename = ['3D_Raman_weights2'];
saveas(hh,filename)
...
plot(th2,s10./s4,th2,s7./s4);
%errorbar(Ev,Ipp+Imm+Ipm,errs(:,4)+errs(:,5)+errs(:,6));
%title(['Rel. Raman Spectral Weights for HyperHoneycomb lowest spinons h=0'])
xlabel('J_x/J_z');
ylabel('Rel. Spectral Weight');
legend({'I^-_{aa}/I^-_{ac}','I^-_{ab}/I^-_{ac}'}, 'Location', 'NorthEast');
hold off;
filename = ['3D_Raman_weights_lowband2'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
