...
for n = 1:nn
    ...
    out = KitaevRaman_H1_2(N,bins,flag,nn,Jx,Jz,h);
...
title(['2p DOS for H1: J_x=',num2str(Jx),', h=',num2str(h),'_2'])
xlabel('\omega/J_z');
ylabel('density of states');
%set(gca,'XTick',-3:3); 
%set(gca,'YTick',2*(0:5));
hold off;
filename = ['3D_2p_DOS_H1_Jx_',num2str(round(100*Jx)),'_h_',num2str(100*h),'_2'];
saveas(hh,filename)
...
plot(Ev/4,D);
%errorbar(Ev,Ipp+Imm+Ipm,errs(:,4)+errs(:,5)+errs(:,6));
title(['DOS for H1: J_x=',num2str(Jx),', h=',num2str(h),'_2'])
xlabel('\omega/J_z');
ylabel('density of states');
%set(gca,'XTick',-3:3); 
%set(gca,'YTick',2*(0:5));
hold off;
filename = ['DOS_H1_Jx_',num2str(round(100*Jx)),'_h_',num2str(100*h),'_2'];
saveas(hh,filename)
print(hh, '-dpng', filename);
print(hh, '-depsc', filename);
